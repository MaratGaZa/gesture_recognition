/**
 * Detects the "point" hand gesture.
...
function isFingerExtended(landmarks, finger) {
  ...
  return distToTip / distToBase > 1.25;
}
...
export function isPoint(landmarks) {
  if (!landmarks || landmarks.length < 21) {
    return false;
  }

  const thumbFolded = isFingerFolded(landmarks, FINGER_INDICES.thumb);
